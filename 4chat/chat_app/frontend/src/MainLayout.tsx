...
const MainLayout: React.FC<MainLayoutProps> = ({ user, onLogout, authenticatedFetch }) => {
  ...
  useEffect(() => {
    if (selectedRoom) {
      const loadMessages = async () => {
        try {
          const response = await authenticatedFetch(`/rooms/${selectedRoom.id}/messages`);
          const data = await response.json();
  ...
  const handleInvitationAction = async (invitationId: number, action: string) => {
    try {
      const response = await authenticatedFetch(`/invitations/${invitationId}/action`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleSendInvitation = async () => {
    ...
    try {
      ...
      const userResponse = await authenticatedFetch(`/users/${inviteeUsername}`);
      if (!userResponse.ok) {
      ...
      // 然后发送邀请
      const inviteResponse = await authenticatedFetch('/invitations', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
