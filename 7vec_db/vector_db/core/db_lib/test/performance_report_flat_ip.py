#!/usr/bin/env python3
"""
FLAT_IP (Inner Product) performance report comparing C++/Rust implementations
"""
...
try:
    from vectordb.cpp import _flat_ip
    cpp_available = True
except ImportError:
    cpp_available = False
...
def test_flat_ip_performance(dimensions=128, num_vectors=100000, num_queries=100, k=10):
    ...
    if cpp_available:
        print("\nTesting C++ FLAT_IP implementation:")
        try:
            start_time = time.time()
            index = _flat_ip.IndexFlatIP(dimensions)
...
def run_comprehensive_test():
    ...
    for dim, num_vec, num_q, k in test_configs:
        test_flat_ip_performance(dim, num_vec, num_q, k)
