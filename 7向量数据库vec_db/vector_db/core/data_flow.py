...
class DataFlowManager:
    ...
    def _init_file_storage(self) -> FileStorageInterface:
        ...
        if test_mode:
            ...
            class InMemoryFileStorage(FileStorageInterface):
                ...
                def store_file(self, file_content: bytes, file_path: str) -> str:
                    import uuid
                    file_id = str(uuid.uuid4())
                    self.files[file_id] = (file_content, file_path)
                    return file_id
                ...
                def read_file(self, file_path: str) -> bytes:
                    # 查找文件路径对应的文件内容
                    for file_content, stored_path in self.files.values():
                ...
                def list_files(self) -> List[str]:
                    return [file_path for _, file_path in self.files.values()]
    ...
    def _init_metadata_storage(self) -> MetadataStorageInterface:
        ...
        if test_mode:
            # 在测试模式下使用内存存储
            class InMemoryStorage(MetadataStorageInterface):
                def __init__(self):
                    self.metadata = {}
    ...
    def process_file(self, file_path: str, collection_id: str) -> Dict[str, Any]:
        ...
        with open(file_path, 'rb') as f:
            content = f.read()
        stored_file_id = self.file_storage.store_file(file_path, content)
        ...
        self.vector_db.insert(vectors, chunk_metadata, collection_id)
...
# 全局数据流程管理器实例
data_flow_manager = DataFlowManager(test_mode=True)