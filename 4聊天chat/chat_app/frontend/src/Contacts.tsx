...
const Contacts: React.FC<ContactsProps> = ({ user, onLogout, authenticatedFetch }) => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const response = await authenticatedFetch(`/users/search?query=${encodeURIComponent(searchQuery)}`);
      if (response.ok) {
        const data = await response.json();
  ...
  const handleAddContact = async (userId: number) => {
    try {
      const response = await authenticatedFetch('/contact-requests', {
        method: 'POST',
              ...
              {searchResults.map((user) => (
                <li key={user.id} style={{ 
                  ...
                  <div style={{ 
                    ...
                    <img src={user.avatar || '/static/avatars/default/default1.png'} alt={user.displayname} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                  </div>
                  <div style={{ flex: 1 }}>
